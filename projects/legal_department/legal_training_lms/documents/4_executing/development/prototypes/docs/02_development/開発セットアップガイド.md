...
`development/prototypes` ディレクトリで以下を実行します。

```bash
# デプロイ実行（clasp push + clasp deploy）
./scripts/deploy.sh
```
...
**注意**: `.clasp.json` が prototypes に存在すること、および Web アプリとしてのデプロイが一度作成済みである必要があります。初回は Apps Script エディタで「デプロイ」→「新しいデプロイ」→「Web アプリ」を選択して作成してください。
